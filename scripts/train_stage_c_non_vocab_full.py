...
DEFAULT_CONFIG_PATH = ROOT / "configs" / "stage_c_non_vocab_aug_v2_full_20epoch_bf16.yaml"
DEFAULT_OUTPUT_DIR = ROOT / "outputs" / "stage_c_non_vocab_aug_v2_full_30epoch"
...
EPOCH_METRIC_CSV_FIELDS = (
    ...
    "formal_eval_full_decode",
    "formal_eval_dedup_retrieval",
    "full_epoch_decode",
    "dedup_epoch_retrieval",
    "early_stopping_metric",
    "early_stopping_mode",
    "early_stopping_monitor_only",
    "early_stopping_current",
    "early_stopping_best",
    "early_stopping_best_checkpoint",
    "early_stopping_wait",
    "early_stopping_stop_training",
    "early_stopping_would_stop_training",
...
class StageCConfig:
    ...
    early_stopping_metric: str = "restore_loss"
    ...
    formal_eval_dedup_retrieval: bool = False
...
def collate_stage_c_records(
    ...
    *,
    ...
    for graph_index, row in enumerate(records):
        ...
        view_ids.append(str(row.get("view_id", "")))
        augmentation_strategies.append(str(row.get("augmentation_strategy") or row.get("text_view_1_strategy") or "identity"))
        text_view_1_strategies.append(str(row.get("text_view_1_strategy", "")))
        text_view_1_values.append(str(row.get("text_view_1", "")))
    ...
    return StageCBatch(
        ...
        text_view_1=text_view_1_values,
...
def unique_record_sample_limit(dataset: Any) -> int:
    rows = getattr(dataset, "rows", None)
    if rows is None:
        return len(dataset)
    return len({str(row.get("record_id")) for row in rows})


def formal_eval_decode_sample_limit(config: StageCConfig, dataset: Any, configured_limit: int) -> int:
    if config.formal_eval_full_decode:
        return full_decode_sample_limit(dataset)
    return configured_limit


def formal_eval_retrieval_sample_limit(config: StageCConfig, dataset: Any, configured_limit: int) -> int:
    if config.formal_eval_dedup_retrieval:
        return unique_record_sample_limit(dataset)
    return configured_limit


def restore_prediction_context(batch: StageCBatch, row_index: int) -> dict[str, Any]:
    return {
...
def evaluate_stage_c(
    *,
    ...
    dedup_retrieval: bool | None = None,
    strategy_aggregate_prefix: str | None = "all_view",
    ...
    restore_predictions: list[dict[str, Any]] = []
    retrieval_ids: list[str] = []
    retrieval_seen_record_ids: set[str] = set()
        ...
        for batch_index, batch in enumerate(dataloader):
            ...
            if remaining_retrieval:
                if dedup_retrieval:
                    for row_index, record_id in enumerate(batch.record_ids):
                        if len(retrieval_ids) >= retrieval_sample_limit:
                            break
                        if record_id in retrieval_seen_record_ids:
                            continue
                        retrieval_seen_record_ids.add(record_id)
                        retrieval_ids.append(record_id)
                        retrieval_text.append(output.z_text[row_index : row_index + 1].detach().cpu())
                        retrieval_graph.append(output.z_graph[row_index : row_index + 1].detach().cpu())
                else:
            ...
            if remaining_decode:
                ...
                for row_index, token_ids in enumerate(decoded_ids.detach().cpu().tolist()):
                    ...
                    restore_predictions.append(prediction)
    ...
    retrieval_rows = retrieval_predictions(z_text, z_graph, retrieval_ids)
    decode_denom = max(decoded_count, 1)
    restore_loss_mean = sum(restore_losses) / max(len(restore_losses), 1)
    align_loss_mean = sum(align_losses) / max(len(align_losses), 1)
    weighted_align_loss = config.align_loss_weight * align_loss_mean
    metrics = {
        "loss": sum(total_losses) / max(len(total_losses), 1),
        "restore_loss": restore_loss_mean,
        "align_loss": align_loss_mean,
        "weighted_align_loss": weighted_align_loss,
        "align_to_restore_ratio": weighted_align_loss / restore_loss_mean if restore_loss_mean > 0 else 0.0,
    ...
    if strategy_aggregate_prefix is not None:
        metrics = add_strategy_aggregates(metrics, restore_predictions, prefix=strategy_aggregate_prefix)
    return metrics, failed_cases, restore_predictions, retrieval_rows
...
def write_eval_report(path: Path, metrics: dict[str, Any], config: StageCConfig) -> None:
    lines = [
        ...
        "",
        f"This checkpoint is a Stage C artifact for `L_restore + {config.align_loss_weight} * L_align`.",
        "",
        ...
        flat_row.update(
            {
                ...
                "early_stopping_monitor_only": early_stopping.get("monitor_only"),
                ...
                "early_stopping_would_stop_training": early_stopping.get("would_stop_training"),
...
def save_stage_c_artifacts(
    *,
    ...
    with (output_dir / "predictions.jsonl").open("w", encoding="utf-8") as handle:
        for row in restore_predictions:
            handle.write(json.dumps(row, ensure_ascii=False, sort_keys=True) + "\n")
    ...
    *,
    ...
    for split in ("valid", "test"):
        ...
        metrics, failed_cases, predictions, retrieval_rows = evaluate_stage_c(
            ...
            strategy_aggregate_prefix=output_prefix,
        )
...
def validate_training_config(config: StageCConfig) -> None:
    if config.per_device_train_batch_size < 2:
        raise ValueError("Stage C InfoNCE training requires per_device_train_batch_size >= 2")
...
def run_reload_check(
    *,
    ...
    reload_loader = DataLoader(valid_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn)
    batch = next(iter(reload_loader)).to(device)
    with torch.no_grad():
        output = forward_stage_c(
    ...
    (output_dir / "reload_check.json").write_text(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Stage C non-vocab BaseLite warmup full run.")
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise SystemExit("Stage C Qwen2.5-7B bf16 LoRA training requires a CUDA GPU.")
    ...
    print(json.dumps({"output_dir": str(output_dir), "metrics": metrics, "reload_check": reload_check, "extra_eval": extra_eval}, ensure_ascii=False, indent=2, sort_keys=True))
