# Stage C Non-vocab Full 3-epoch Cloud Run

This package is for the first longer full-data Stage C run:

```text
text_view + repeat_unit_graph -> L_restore + L_align -> checkpoint/reload/eval
```

It uses the same code path that passed the Stage C smoke run, removes the sample
caps, and trains for 3 epochs. Epoch-end checkpoints are saved and validated so
we can compare whether restore and text-graph retrieval improve across epochs.
...
- epoch-end checkpoint snapshots with validation metrics
...
```text
packaging/stage_c_non_vocab_full_3epoch_files.txt
```
...
- Recommended free disk space: at least 120 GB on the data disk for package,
  model cache, logs, checkpoints, and final outputs.
...
```text
/root/autodl-tmp/baselite_stage_c_full_3epoch/
  packages/
  work/
  venv/
  hf_cache/
...
```bash
python3 -m venv --system-site-packages /root/autodl-tmp/baselite_stage_c_full_3epoch/venv
source /root/autodl-tmp/baselite_stage_c_full_3epoch/venv/bin/activate
...
```bash
export HF_HOME=/root/autodl-tmp/baselite_stage_c_full_3epoch/hf_cache
export TRANSFORMERS_CACHE=/root/autodl-tmp/baselite_stage_c_full_3epoch/hf_cache/transformers
```
...
python scripts/train_stage_c_non_vocab_smoke.py \
  --config configs/stage_c_non_vocab_full_3epoch_bf16.yaml \
  --model-name-or-path Qwen/Qwen2.5-7B \
  --output-dir outputs/stage_c_non_vocab_full_3epoch
```
...
estimated optimizer steps: 1737
quick eval every: 200 optimizer steps
epoch checkpoints: epoch_001, epoch_002, epoch_003
checkpoint eval: 256 valid rows, 64 decode samples, 256 retrieval samples
...
```text
outputs/stage_c_non_vocab_full_3epoch/
  eval_metrics.json
  eval_report.md
  failed_cases.jsonl
  ...
  projectors/
  reload_smoke.json
  stage_c_non_vocab_full_3epoch_bf16.yaml
  tokenizer/
...
`reload_smoke.json` in the final output should report `"status": "passed"`.

## Acceptance Bar

Treat this run as successful if:

- all losses are finite;
- `train_loss_decreased` is `true`;
- `reload_smoke.status == "passed"`;
- epoch checkpoints exist and contain validation metrics;
- restore and retrieval metrics are computable at every checkpoint;
- output artifacts are complete;
- text-to-graph and graph-to-text top-k can be compared against the smoke
  baseline.

This is still not the final BaseLite checkpoint. It is the first longer
non-vocab Stage C training pass.
...
- over-generation severity by checkpoint;
- whether epoch_001 -> epoch_003 improves decode validity or retrieval top-k.
